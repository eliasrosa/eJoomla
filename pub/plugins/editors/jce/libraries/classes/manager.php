...
class Manager extends JContentEditorPlugin 
{
	...
	*/
	function getRootDir()
	{
		...
		$jdir = array('administrator', 'cache', 'components', 'includes', 'language', 'libraries', 'logs', 'media', 'modules', 'plugins', 'templates', 'xmlrpc');
		...
		$parts = explode('/', $base);
		if (in_array(strtolower($parts[0]), $jdir)) {
			$base = 'images/stories';
		}		
		...
		if ($this->isSuperAdmin()) {
			...
			$base = preg_replace(array('/\$id/', '/\$username/', '/\$usertype/', '/\$group/'), array($user->id, strtolower($user->username), strtolower($user->usertype), strtolower($this->group->name)), $base);
		}			
		// Clean
	...
	*/
	...
	{
		return preg_replace(array('/%2F/', '/%3F/', '/%40/', '/%2A/', '/%2B/'), array('/', '?', '@', '*', '+'), rawurlencode($string));
	}
	/**
	 * Build a tree list
	 * @param string $dir The relative path of the folder to search        
	...
	*/
	...
	{									
		...
		if ($folders) {
			...
			foreach ($folders as $folder) {
				...
				$result .= '<li id="'. $this->escape($folder['url']) .'"><div class="tree-row"><div class="tree-image'. $open .'"></div><span class="folder'. $open .'"><a href="javascript:;">'. $folder['name'] .'</a></span></div>';
				if($open){
	...
	{			
		...
		if ($folders) {
			foreach ($folders as $folder) {
				$classes 	= array();
				$classes[] 	= is_writable(Utils::makePath($this->getBaseDir(), utf8_decode($folder['url']))) || $this->isFtp() ? 'writable' : 'notwritable';
				$classes[] 	= preg_match('/[^a-zA-Z0-9:\.\_\-]/', $folder['name']) ? 'notsafe' : 'safe';
		...
		if ($files) {
			foreach ($files as $file) {
				$classes 	= array();
				$classes[] 	= is_writable(Utils::makePath($this->getBaseDir(), utf8_decode($file['url']))) || $this->isFtp() ? 'writable' : 'notwritable';
				$classes[] 	= preg_match('/[^a-zA-Z0-9\.\_\-]/', $file['name']) ? 'notsafe' : 'safe';
	...
	function fileDelete($files)
	{
		...
		foreach ($files as $file) {
			$path = Utils::makePath($this->getBaseDir(), utf8_decode($file));
	...
	{
		jimport('joomla.filesystem.folder');
		$folder = Utils::makePath($this->getBaseDir(), utf8_decode(rawurldecode($relative)));
	...
	*/
	...
	{			            
		jimport('joomla.filesystem.file');
		$src = Utils::makePath($this->getBaseDir(), utf8_decode(rawurldecode($src)));
	...
	*/
	...
	{			
		...
		$files = explode(",", rawurldecode($files));
		foreach ($files as $file) {
			$filesrc 	= Utils::makePath($this->getBaseDir(), utf8_decode($file));
	...
	*/
	...
	{						
		$dir = Utils::makePath(utf8_decode(rawurldecode($dir)), Utils::makeSafe($new));
