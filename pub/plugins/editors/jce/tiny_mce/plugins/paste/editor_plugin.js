 ...
 * Additional modifications - Ryan Demmer
	...
	tinymce.create('tinymce.plugins.PastePlugin', {
		...
		_openWin : function(cmd){
			...
			ed.windowManager.open({
				file : t.url + (cmd == 'mcePasteText' ? '/pastetext.htm' : '/paste.htm'),
		...
		_preProcess : function(pl, o) {
			...
			if (/(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/.test(h)) {
				...
				process([
					...
					/ (id|name|language|type|on\w+|v:\w+)=\"([^\"]*)\"/gi,				// on.., language attributes with quotes "
					/ (id|name|language|type|on\w+|v:\w+)=(\w+)/gi,						// on.., language attributes without quotes (IE)
